...
const TransactionView = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const response = await fetch('http://localhost:8000/api/categories-structure/');
          if (response.ok) {
  ...
  const saveTransaction = async () => {
    try {
      ...
      // Send update request
      const response = await fetch(`http://localhost:8000/api/transactions/${editingTransaction.id}`, {
        method: 'PUT',
        headers: {
  ...
  const deleteTransaction = async () => {
    ...
    try {
      ...
      // Send delete request
      const response = await fetch(`http://localhost:8000/api/transactions/${transactionToDelete.id}`, {
        method: 'DELETE',
      });
